...
this.space=""+this.options.app_id+"/"+this.name;this.username=this.options.username;this.namespace=""+this.username+":"+this.space;this.clientid=null;try{this.clientid=localStorage.getItem(""+this.namespace+"/clientid")}catch(j){this.clientid=null}if(this.clientid==null||this.clientid.indexOf("sjs")!==0){this.clientid="sjs-"+this.s.bversion+"-"+this.uuid(5);try{localStorage.setItem(""+this.namespace+"/clientid")}catch(m){}}this.cb_events=["notify","notify_init","notify_version","local","get","ready",
"notify_pending","error"];this.cbs={};this.cb_e=this.cb_l=this.cb_ni=this.cb_np=null;this.cb_n=this.cb_nv=this.cb_r=function(){};this.authorized=this.initialized=false;this.data={last_cv:0,ccid:this.uuid(),store:{},send_queue:[],send_queue_timer:null};this.started=false;if("nostore"in this.options)this.loaded=0;else{this._load_meta();this.loaded=this._load_data()}this._last_pending=null;this._send_backoff=15E3;this._backoff_max=12E4;this._backoff_min=15E3}d.prototype.S4=function(){return((1+Math.random())*
65536|0).toString(16).substring(1)};d.prototype.uuid=function(a){var b;a=a||8;for(b=this.S4();a-=1;)b+=this.S4();return b};d.prototype.now=function(){return(new Date).getTime()};d.prototype.on=function(a,b){if(g.call(this.cb_events,a)>=0){this.cbs[a]=b;switch(a){case "get":case "local":return this.cb_l=b;case "notify":return this.cb_n=b;case "notify_version":return this.cb_nv=b;case "notify_pending":return this.cb_np=b;case "notify_init":return this.cb_ni=b;case "ready":return this.cb_r=b;case "error":return this.cb_e=
b}}else throw Error("unsupported callback event");};d.prototype.show_data=function(){var a,b;if(this.s.supports_html5_storage()){b=0;for(a in localStorage)if(k.call(localStorage,a))b+=1;return false}};d.prototype._save_meta=function(){if(!this.s.supports_html5_storage())return false;try{localStorage.setItem(""+this.namespace+"/ccid",this.data.ccid);localStorage.setItem(""+this.namespace+"/last_cv",this.data.last_cv)}catch(a){return false}return true};d.prototype._load_meta=function(){var a;if(this.s.supports_html5_storage()){this.data.ccid=
localStorage.getItem(""+this.namespace+"/ccid");if((a=this.data).ccid==null)a.ccid=1;this.data.last_cv=localStorage.getItem(""+this.namespace+"/last_cv");if((a=this.data).last_cv==null)a.last_cv=0;return false}};d.prototype._verify=function(a){if(!("object"in a)||!("version"in a))return false;if(this.jd.entries(a.object)===0)return"last"in a&&this.jd.entries(a.last)>0?true:false;else if(a.version==null)return false;return true};d.prototype._load_data=function(){var a,b,h,j,m,p,q,s;if(this.s.supports_html5_storage()){p=
""+this.namespace+"/e/";m=p.length;j=0;if(localStorage.length===0)return 0;b=q=0;for(s=localStorage.length-1;0<=s?q<=s:q>=s;b=0<=s?++q:--q){h=localStorage.key(b);if(h!=null&&h.substr(0,m)===p){b=h.substr(m,h.length-m);try{a=JSON.parse(localStorage.getItem(h))}catch(w){a=null}if(a!=null)if(this._verify(a)){a.check=null;this.data.store[b]=a;j+=1}else this._remove_entity(b)}}return j}};d.prototype._save_entity=function(a){var b,h;if(!this.s.supports_html5_storage())return false;h=""+this.namespace+"/e/"+
a;a=this.data.store[a];b=JSON.stringify(a);try{localStorage.setItem(h,b)}catch(j){return false}h=JSON.parse(localStorage.getItem(h));return this.jd.equals(a,h)?true:false};d.prototype._remove_entity=function(a){if(!this.s.supports_html5_storage())return false;a=""+this.namespace+"/e/"+a;try{localStorage.removeItem(a)}catch(b){return false}return true};d.prototype.start=function(){var a,b;this.namespace=""+this.username+":"+this.space;this.started=true;this.first=false;if(this.authorized)if(this.initialized)return this.retrieve_changes();
else{if(!this.first){this.notify_index={};return this._refresh_store()}}else if(this.s.connected){this.first=true;a="limit"in this.options?"i:1:::"+this.options.limit:"i:1:::40";this.irequest_time=this.now();this.index_request=true;this.notify_index={};b={app_id:this.options.app_id,token:this.options.token,name:this.name,clientid:this.clientid,build:this.s.bversion};if(!this.initialized)b.cmd=a;this.send("init:"+JSON.stringify(b))}};d.prototype.on_data=function(a){var b,h,j;if(a.substr(0,5)==="auth:"){b=
a.substr(5);if(b==="expired"){this.started=false;this.cb_e!=null&&this.cb_e("auth")}else{this.username=b;this.authorized=true;if(this.initialized)return this.start()}}else if(a.substr(0,4)==="cv:?")return setTimeout(function(m){return function(){return m._refresh_store()}}(this));else if(a.substr(0,2)==="c:"){b=JSON.parse(a.substr(2));if(this.data.last_cv==="0"&&b.length===0&&!this.cv_check){this.cv_check=true;this._refresh_store()}return this.on_changes(b)}else if(a.substr(0,2)==="i:")return this.on_index_page(JSON.parse(a.substr(2)));
else if(a.substr(0,2)==="e:"){j=a.indexOf("\n");h=a.substr(2,j-2);b=h.substr(h.lastIndexOf(".")+1);h=h.substr(0,h.lastIndexOf("."));a=a.substr(j+1);if(a==="?")return this.on_entity_version(null,h,b);else{a=JSON.parse(a);return this.on_entity_version(a.data,h,b)}}else return false};d.prototype.send=function(a){return this.s.send(""+this.chan+":"+a)};d.prototype._refresh_store=function(){this.send("limit"in this.options?"i:1:::"+this.options.limit:"i:1:::40");this.irequest_time=this.now();this.index_request=
true};d.prototype.on_index_page=function(a){var b,h,j,m,p,q;this.now();h=0;q=a.index;m=0;for(p=q.length;m<p;m++){b=q[m];this.notify_index[b.id]=false;h++;setTimeout(function(s){return function(w){return function(){return s.on_entity_version(w.d,w.id,w.v)}}}(this)(b))}if(!("mark"in a)||"limit"in this.options){this.index_request=false;if("current"in a){this.data.last_cv=a.current;this._save_meta()}else this.data.last_cv=0;if(h===0)return this._index_loaded()}else{this.index_request=true;a=a.mark;j=
function(s){return function(w){s.send("i:1:"+w+"::100");return s.irequest_time=s.now()}}(this);return setTimeout(function(){return function(s){return function(){return j(s)}}}(this)(a),this.options.page_delay)}};d.prototype.on_index_error=function(){return false};d.prototype.load_versions=function(a,b){var h,j,m,p,q;if(!(a in this.data.store))return false;h=Math.max(this.data.store[a].version-(b+1),1);q=[];j=m=h;for(p=this.data.store[a].version-1;h<=p?m<=p:m>=p;j=h<=p?++m:--m)q.push(this.send("e:"+
a+"."+j));return q};d.prototype.get_version=function(a,b){return this.send("e:"+(""+a+"."+b))};d.prototype.on_entity_version=function(a,b,h){var j,m,p;j=a!=null?this.jd.deepCopy(a):null;p=this.initialized===false&&this.cb_ni!=null?this.cb_ni:this.cb_n;if(b in this.data.store&&"last"in this.data.store[b]&&this.jd.entries(this.data.store[b].last)){j=this.jd.deepCopy(this.data.store[b].last);p(b,j,null);this.notify_index[b]=true;return this._check_update(b)}else if(b in this.data.store&&h<this.data.store[b].version)return this.cb_nv(b,
j,h);else{b in this.data.store||(this.data.store[b]={});this.data.store[b].id=b;this.data.store[b].object=a;this.data.store[b].version=parseInt(h);p(b,j,h);this.notify_index[b]=true;a=0;b=this.notify_index;for(m in b)k.call(b,m)&&this.notify_index[m]===false&&a++;if(a===0&&this.index_request===false)return this._index_loaded()}};d.prototype._index_loaded=function(){this.initialized===false&&this.cb_r();this.initialized=true;return this.retrieve_changes()};d.prototype._notify_client=function(a,b,h,
j,m){var p,q,s,w,v;if(this.cb_l==null)this.cb_n(a,b,m);else{p=this.cb_l(a);q=null;v=[];if(this.jd.typeOf(p)==="array"){s=p[2];w=p[1];p=p[0];if(q=this.s._captureCursor(s)){v[0]=q.startOffset;if("endOffset"in q)v[1]=q.endOffset}}if(p!=null&&h!=null){p=this.jd.object_diff(h,p);if(this.jd.entries(p)===0){j=j;b=h}else{j=this.jd.transform_object_diff(p,j,h);b=b}if(q){b=this.jd.apply_object_diff_with_offsets(b,j,w,v);if(s!=null&&"value"in s)s.value=b[w];q.startOffset=v[0];if(v.length>1){q.endOffset=v[1];
if(q.startOffset>=q.endOffset)q.collapsed=true}this.s._restoreCursor(s,q)}else b=this.jd.apply_object_diff(b,j);return this.cb_n(a,b,m)}else return b?this.cb_n(a,b,m):this.cb_n(a,null,null)}};d.prototype._check_update=function(a){var b,h,j,m,p;if(!(a in this.data.store))return false;h=this.data.store[a];if(h.change){a=false;p=this.data.send_queue;j=0;for(m=p.length;j<m;j++){b=p[j];if(b.id===h.change.id&&b.ccid===h.change.ccid)a=true}if(!a){this._queue_change(h.change);return true}return false}if(h.check!=
null)return false;if("last"in h&&this.jd.equals(h.object,h.last)){delete h.last;this._remove_entity(a);return false}b=this._make_change(a);if(b!=null){h.change=b;this._queue_change(b)}else this._remove_entity(a);return true};d.prototype.update=function(a,b){var h,j,m,p,q;if(arguments.length===1)if(this.cb_l!=null){b=this.cb_l(a);if(this.jd.typeOf(b)==="array")b=b[0]}else throw Error("missing 'local' callback");if(a==null&&b==null)return false;if(a!=null){if(a.length===0||a.indexOf("/")!==-1)return false}else a=
this.uuid();a in this.data.store||(this.data.store[a]={id:a,object:{},version:null,change:null,check:null});j=this.data.store[a];j.last=this.jd.deepCopy(b);j.modified=this.s._time();this._save_entity(a);q=this.data.send_queue;m=0;for(p=q.length;m<p;m++){h=q[m];if(String(a)===h.id)return null}j.check!=null&&clearTimeout(j.check);j.check=setTimeout(function(s){return function(w,v){return function(){v.check=null;v.change=s._make_change(w);delete v.last;s._save_entity(w);return s._queue_change(v.change)}}}(this)(a,
j),this.options.update_delay);return a};d.prototype._make_change=function(a){var b,h;h=this.data.store[a];b={id:String(a),ccid:this.uuid()};if(this.initialized)if(this.cb_l!=null){a=this.cb_l(a);if(this.jd.typeOf(a)==="array")a=a[0]}else if("last"in h)a=h.last;else return null;else if("last"in h)a=h.last;else return null;if(h.version!=null)b.sv=h.version;if(a===null&&h.version!=null)b.o="-";else if(a!=null&&h.object!=null){b.o="M";if("sendfull"in h){b.d=this.jd.deepCopy(a);delete h.sendfull}else{b.v=
this.jd.object_diff(h.object,a);if(this.jd.entries(b.v)===0)b=null}}else b=null;return b};d.prototype._queue_change=function(a){if(a!=null){this.data.send_queue.push(a);this.send("c:"+JSON.stringify(a));this._check_pending();this.data.send_queue_timer!=null&&clearTimeout(this.data.send_queue_timer);return this.data.send_queue_timer=setTimeout(this._send_changes,this._send_backoff)}};d.prototype._send_changes=function(){var a,b,h,j;if(this.data.send_queue.length===0)this.data.send_queue_timer=null;
else{if(this.s.connected){j=this.data.send_queue;b=0;for(h=j.length;b<h;b++){a=j[b];this.send("c:"+JSON.stringify(a))}}this._send_backoff*=2;if(this._send_backoff>this._backoff_max)this._send_backoff=this._backoff_max;return this.data.send_queue_timer=setTimeout(this._send_changes,this._send_backoff)}};d.prototype.retrieve_changes=function(){this.send("cv:"+this.data.last_cv)};d.prototype.on_changes=function(a){var b,h,j,m,p,q,s,w,v,u,r,x,B;h=[];w=false;this._send_backoff=this._backoff_min;v=0;for(r=
a.length;v<r;v++){b=a[v];j=b.id;m=[];B=this.data.send_queue;u=0;for(x=B.length;u<x;u++){s=B[u];if(b.clientid===this.clientid&&j===s.id){b.local=true;m.push(s);h.push(j)}}s=0;for(u=m.length;s<u;s++){q=m[s];this.data.store[q.id].change=null;this._save_entity(q.id);this.data.send_queue=function(){var z,I,J,E;J=this.data.send_queue;E=[];z=0;for(I=J.length;z<I;z++){p=J[z];p!==q&&E.push(p)}return E}.call(this)}m.length>0&&this._check_pending();if("error"in b)switch(b.error){case 412:j=h.indexOf(b.id);j>
-1&&h.splice(j,1);break;case 409:break;case 405:if(b.id in this.data.store)this.data.store[b.id].version=null;w=true;break;case 440:this.data.store[j].sendfull=true;break;default:if(b.id in this.data.store)this.data.store[b.id].version=null;w=true}else{m=b.o;if(m==="-"){delete this.data.store[j];this._remove_entity(j);"local"in b||this._notify_client(b.id,null,null,null,null)}else if(m==="M"){m=this.data.store[j];if("sv"in b&&m!=null&&m.version!=null&&m.version===b.sv||!("sv"in b)||b.ev===1){if(m==
null){this.data.store[j]={id:j,object:{},version:null,change:null,check:null};m=this.data.store[j]}j=this.jd.deepCopy(m.object);m.object=this.jd.apply_object_diff(m.object,b.v);m.version=b.ev;m=this.jd.deepCopy(m.object);"local"in b||this._notify_client(b.id,m,j,b.v,b.ev)}else if(!(m!=null&&m.version!=null&&b.ev<=m.version)){if(m!=null)this.data.store[j].version=null;w=true}}if(!w){this.data.last_cv=b.cv;this._save_meta()}}}if(w)setTimeout(function(z){return function(){return z._refresh_store()}}(this));
else{a=function(z){return function(I){return setTimeout(function(){return z._check_update(I)},z.options.update_delay)}}(this);w=0;for(v=h.length;w<v;w++){j=h[w];a(j)}}};d.prototype.pending=function(){var a,b,h,j,m;(function(){var p,q,s,w;s=this.data.send_queue;w=[];p=0;for(q=s.length;p<q;p++){a=s[p];w.push(a.id)}return w}).call(this);j=this.data.send_queue;m=[];b=0;for(h=j.length;b<h;b++){a=j[b];m.push(a.id)}return m};d.prototype._check_pending=function(){var a,b,h,j,m,p;if(this.cb_np!=null){a=this.pending();
b=true;if(this._last_pending){b=false;if(this._last_pending.length===a.length){p=this._last_pending;j=0;for(m=p.length;j<m;j++){h=p[j];if(a.indexOf(h)===-1)b=true}}else b=true}if(b){this._last_pending=a;return this.cb_np(a)}}};return d}();f=function(){function d(a,b){var h,j,m;this.app_id=a;this.options=b;this._restoreCursor=c(this._restoreCursor,this);this._captureCursor=c(this._captureCursor,this);this._sock_message=c(this._sock_message,this);this._sock_hb_check=c(this._sock_hb_check,this);this._sock_closed=
c(this._sock_closed,this);this._sock_opened=c(this._sock_opened,this);this._sock_connect=c(this._sock_connect,this);this.synced=c(this.synced,this);this.send=c(this.send,this);this.stop=c(this.stop,this);this.start=c(this.start,this);this.on=c(this.on,this);this.bucket=c(this.bucket,this);this.bversion=2014030401;this.jd=new jsondiff;this.dmp=this.jd.dmp;this.auth_token=null;this.options=this.options||{};this.options.app_id=this.app_id;this.sock_opts={debug:false};if("sockjs"in this.options){m=this.options.sockjs;
for(h in m)if(k.call(m,h)){j=m[h];this.sock_opts[h]=j}}"host"in this.options||(this.options.host="api.simperium.com");"port"in this.options||(this.options.port=80);if("token"in this.options)this.auth_token=this.options.token;h=this.options.host.indexOf("simperium.com")!==-1?"https":"http";this.buckets={};this.channels=0;"update_delay"in this.options||(this.options.update_delay=0);"page_delay"in this.options||(this.options.page_delay=0);this.options.prefix="sock/1/"+this.app_id;this.options.port=parseInt(this.options.port);
this.sock_url=this.options.port!==80&&this.options.port!==443?""+h+"://"+this.options.host+":"+this.options.port+"/"+this.options.prefix:""+h+"://"+this.options.host+"/"+this.options.prefix;this.stopped=false;this._sock_backoff=3E3;this._sock_hb=1;this._sock_connect()}d.prototype.lowerstrip=function(a){a=a.toLowerCase();return String.prototype.trim!=null?a.trim():a.replace(/^\s+|\s+$/g,"")};d.prototype._time=function(){var a;a=new Date;return Date.UTC(a.getUTCFullYear(),a.getUTCMonth(),a.getUTCDate(),
a.getUTCHours(),a.getUTCMinutes(),a.getUTCSeconds(),a.getUTCMilliseconds())/1E3};d.prototype.supports_html5_storage=function(){try{return"localStorage"in window&&window.localStorage!==null}catch(a){return false}};d.prototype.bucket=function(a,b){var h;a=this.lowerstrip(a);b=b||{};b.n=this.channels++;h=new e(this,a,b);return this.buckets[b.n]=h};d.prototype.on=function(a,b,h){return this.buckets[a].on(b,h)};d.prototype.start=function(){var a,b,h;this.stopped=false;b=this.buckets;h=[];for(a in b)if(k.call(b,
a)){e=b[a];h.push(e.start())}return h};d.prototype.stop=function(){this.stopped=true;if(this.sock!=null)return this.sock.close()};d.prototype.send=function(a){return this.sock.send(a)};d.prototype.synced=function(){var a,b;b=this.buckets;for(a in b)if(k.call(b,a)){e=b[a];if(e.pending().length>0)return false}return true};d.prototype._sock_connect=function(){this.authorized=this.connected=false;this.sock=new SockJS(this.sock_url,void 0,this.sock_opts);this.sock.onopen=this._sock_opened;this.sock.onmessage=
this._sock_message;return this.sock.onclose=this._sock_closed};d.prototype._sock_opened=function(){var a,b,h;this._sock_backoff=3E3;this.connected=true;this._sock_hb_timer=setTimeout(this._sock_hb_check,2E4);b=this.buckets;h=[];for(a in b)if(k.call(b,a)){e=b[a];e.started?h.push(e.start()):h.push(void 0)}return h};d.prototype._sock_closed=function(){var a,b;this.connected=false;b=this.buckets;for(a in b)if(k.call(b,a)){e=b[a];e.authorized=false}if(this._sock_backoff<4E3)this._sock_backoff+=1;else this._sock_backoff=
15E3;if(this._sock_hb_timer){clearTimeout(this._sock_hb_timer);this._sock_hb_timer=null}if(!this.stopped)return setTimeout(this._sock_connect,this._sock_backoff)};d.prototype._sock_hb_check=function(){var a;a=(new Date).getTime()-this._sock_msg_time;if(this.connected!==false){if(a>4E4)this.sock.close();else a>15E3&&this.sock.send("h:"+this._sock_hb);return this._sock_hb_timer=setTimeout(this._sock_hb_check,2E4)}};d.prototype._sock_message=function(a){var b,h;this._sock_msg_time=(new Date).getTime();
b=a.data;h=b.indexOf(":");a=null;if(h===1&&b.charAt(0)==="h")this._sock_hb=b.substr(2);else{try{a=parseInt(b.substr(0,h));b=b.substr(h+1)}catch(j){a=null}if(a!==null)if(a in this.buckets)return this.buckets[a].on_data(b)}};d.prototype._captureCursor=function(){};d.prototype._captureCursor=function(a){if("activeElement"in a&&!a.activeElement)return null;var b=this.dmp.Match_MaxBits/2,h=a.value,j={};if("selectionStart"in a){try{var m=a.selectionStart,p=a.selectionEnd}catch(q){return null}j.startPrefix=
h.substring(m-b,m);j.startSuffix=h.substring(m,m+b);j.startOffset=m;j.collapsed=m==p;if(!j.collapsed){j.endPrefix=h.substring(p-b,p);j.endSuffix=h.substring(p,p+b);j.endOffset=p}}else{for(m=a;m.parentNode;)m=m.parentNode;if(!m.selection||!m.selection.createRange)return null;h=m.selection.createRange();if(h.parentElement()!=a)return null;m=m.body.createTextRange();j.collapsed=h.text=="";m.moveToElementText(a);if(!j.collapsed){m.setEndPoint("EndToEnd",h);j.endPrefix=m.text;j.endOffset=j.endPrefix.length;
j.endPrefix=j.endPrefix.substring(j.endPrefix.length-b)}m.setEndPoint("EndToStart",h);j.startPrefix=m.text;j.startOffset=j.startPrefix.length;j.startPrefix=j.startPrefix.substring(j.startPrefix.length-b);m.moveToElementText(a);m.setEndPoint("StartToStart",h);j.startSuffix=m.text.substring(0,b);if(!j.collapsed){m.setEndPoint("StartToEnd",h);j.endSuffix=m.text.substring(0,b)}}if("scrollTop"in a){j.scrollTop=a.scrollTop/a.scrollHeight;j.scrollLeft=a.scrollLeft/a.scrollWidth}return j};d.prototype._restoreCursor=
function(){};d.prototype._restoreCursor=function(a,b){this.dmp.Match_Distance=1E3;this.dmp.Match_Threshold=0.9;var h=this.dmp.Match_MaxBits/2,j=a.value,m=b.startPrefix+b.startSuffix,p,q=this.dmp.match_main(j,m,b.startOffset-h);if(q!==null){p=j.substring(q,q+m.length);m=this.dmp.diff_main(m,p,false);q+=this.dmp.diff_xIndex(m,b.startPrefix.length)}var s=null;if(!b.collapsed){m=b.endPrefix+b.endSuffix;s=this.dmp.match_main(j,m,b.endOffset-h);if(s!==null){p=j.substring(s,s+m.length);m=this.dmp.diff_main(m,
p,false);s+=this.dmp.diff_xIndex(m,b.endPrefix.length)}}if(q===null&&s!==null)q=s;else if(q===null&&s===null)q=b.startOffset;if(s===null)s=q;if("selectionStart"in a){a.selectionStart=q;a.selectionEnd=s}else{for(h=a;h.parentNode;)h=h.parentNode;if(!h.selection||!h.selection.createRange)return;j=a.value.substring(0,q);j=j.replace(/\r\n/g,"\n").length;h=h.body.createTextRange();h.moveToElementText(a);h.collapse(true);h.moveStart("character",j);if(!b.collapsed){j=a.value.substring(q,s);q=j.replace(/\r\n/g,
"\n").length;h.moveEnd("character",q)}h.select()}if("scrollTop"in b){a.scrollTop=b.scrollTop*a.scrollHeight;a.scrollLeft=b.scrollLeft*a.scrollWidth}};return d}();window.Simperium=f;e.prototype.on=e.prototype.on;e.prototype.start=e.prototype.start;e.prototype.load_versions=e.prototype.load_versions;e.prototype.pending=e.prototype.pending;f.prototype.on=f.prototype.on;f.prototype.start=f.prototype.start;f.prototype.bucket=f.prototype.bucket;f.prototype.synced=f.prototype.synced}).call(this);
